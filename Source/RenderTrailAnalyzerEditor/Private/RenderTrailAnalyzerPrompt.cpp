...
namespace UE::RenderTrail::Private
{
	...
	FString LoadRenderTrailAgentSystemPrompt()
	{
		...
		return TEXT("You are RenderTrail's read-only selected-pixel forensics agent. Respond in Chinese with exactly one finish JSON object. Use only supplied RenderDoc evidence. Distinguish the final physical writer from upstream color formation. primaryColorPath and causalLanes are deterministic topology: summarize them but never add, reorder, or invent edges, and return empty lane steps. Treat resourceIndex as identity, neutral-input as non-contributing at this pixel, and compute/UAV producerWrittenValue (Pixel History lastAfter/postMod) as authoritative instead of lastShaderOutput. Keep color, geometry, and overlay semantics separate, never invent shader algorithms or UE asset attribution, and list only real chain breaks in unknowns.");
	}
}

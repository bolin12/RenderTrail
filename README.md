...
4. 点击一个希望解释的像素，该点记为 `P1`，不会被预先认定为正确、正常或异常。再次点击当前像素会将其清除；点击其他位置会直接替换当前点，旧点的证据不会与新点合并。点击 **分析** 确认像素并读取 Pixel History；规则分析完成后，再次点击 **分析** 运行 Agent。单纯选择像素不会启动查询。
5. 右侧面板展示当前像素的结论、因果路径和最多三个嫌疑项。原始事件、资源、Shader 细节以及最近 24 个可追踪跳转默认折叠在 **详细技术证据（Pass / Pipeline / Shader）** 下，只有显式展开才会显示。点击 **清空选点** 会清除当前像素和报告。
...
5. 结果卡片的分析边界止于选中像素，报告最终观测颜色、直接影响该像素的 Event/Pass、已检查的 Pixel/Compute Shader 入口名、有证据支持的 Mesh/资产身份（或 `unknown`）、按顺序排列的像素局部 GPU 过程、置信度和尚未解决的缺口。固定 System Prompt 禁止继续推测 Blueprint、C++、玩法逻辑、状态机或其他上游根因。
...
`RenderTrailAnalyzerEditor` 模块负责像素相关的证据提取、Prompt 和证据预算。Agent 在一次有界请求中接收确定性事件链及已收集的上下文，仅执行语义总结；它不会选择要检查的 RenderDoc 事件。

只有所有选中点都返回完整的 `eventSummaries` 后，请求才会发送；否则 UI 会停止并提示重新构建或重载 Worker。请求内容包括规则结论、有界因果路径、项目/地图元数据、选中坐标、最终观测值、候选事件、带显式完整性标记的近期确定性事件链窗口，以及已经收集的事件上下文。
